...
Each component gets its own folder when it has more than one file (e.g. `ui/table/Table.jsx`, `ui/table/TableRow.jsx`), otherwise a single file is fine.

Keep out of `components/`:
- Data transforms and business logic (sorting, formatting, ratings math) — plain `.js` modules elsewhere (e.g. `teamStatsTableColumns.js`)
